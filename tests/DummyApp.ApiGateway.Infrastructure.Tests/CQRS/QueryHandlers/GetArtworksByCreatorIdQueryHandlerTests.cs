...
public class GetArtworksByCreatorIdQueryHandlerTests
{
    ...
    private GetArtworksByCreatorIdQueryHandler CreateHandler()
        => new GetArtworksByCreatorIdQueryHandler(_storageServiceClientMock.Object, _storageUrlServiceMock.Object);

    [Fact]
    public async Task Handle_ReturnsEmptyList_WhenStorageServiceReturnsNull()
    {
        _storageServiceClientMock
            .Setup(x => x.GetArtworksByCreatorIdAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((IEnumerable<ArtworkDto>?)null);

        var handler = CreateHandler();
        var result = await handler.Handle(new GetArtworksByCreatorIdQuery("creator-1"), CancellationToken.None);
    ...
    public async Task Handle_ReturnsArtworkWithNormalizedUrls_WhenStorageServiceReturnsArtworks()
    {
        var artworks = new[]
        {
            new ArtworkDto { Id = 1, CreatorId = "creator-1", Name = "Test", PublicName = "Test", Description = "desc", CreationDate = DateTime.UtcNow, UploadDate = DateTime.UtcNow, ImgUrl = "blob/path.png", ThumbnailUrl = "small/blob.png", IsActive = true }
        };

        _storageServiceClientMock
            .Setup(x => x.GetArtworksByCreatorIdAsync("creator-1", It.IsAny<CancellationToken>()))
